...
/**
 * estimates — top-level estimate record created by staff
 * Stores customer/vehicle info and links to 2-3 options.
 */
export const estimates = mysqlTable("estimates", {
  id: int("id").autoincrement().primaryKey(),
  /** Unique public-facing ID used in the customer URL */
  publicId: varchar("publicId", { length: 32 }).notNull().unique(),
  /** Status lifecycle */
  status: mysqlEnum("status", ["draft", "sent", "viewed", "approved"]).default("draft").notNull(),
  ...
  advisorName: varchar("advisorName", { length: 128 }),
  location: varchar("location", { length: 128 }),

  // Optional dealer price for savings callout
  dealerPrice: varchar("dealerPrice", { length: 32 }),

  // Staff password for builder access
  builderPassword: varchar("builderPassword", { length: 128 }),

  // Tenant / white-label
  tenantName: varchar("tenantName", { length: 128 }),
  tenantLogo: text("tenantLogo"),
  tenantPhone: varchar("tenantPhone", { length: 32 }),
  tenantColor: varchar("tenantColor", { length: 16 }),
  ...
  approvalSignatureName: varchar("approvalSignatureName", { length: 256 }),
  approvalIp: varchar("approvalIp", { length: 64 }),
...
/**
 * estimate_options — each estimate has 2-3 options (Good/Better/Best)
 */
export const estimateOptions = mysqlTable("estimate_options", {
  id: int("id").autoincrement().primaryKey(),
  estimateId: int("estimateId").notNull(),
  /** Display order (1, 2, 3) */
  sortOrder: int("sortOrder").default(1).notNull(),
  /** Option name: Good / Better / Best / Aftermarket / OEM / Premium or custom */
  name: varchar("name", { length: 64 }).notNull(),
  /** Badge label: Recommended / Most Popular / Best Value / Premium / Budget */
  badge: varchar("badge", { length: 64 }),
  /** Whether this is the highlighted/recommended option */
  isRecommended: int("isRecommended").default(0).notNull(),
  /** Tax rate as a decimal, e.g. 0.07 for 7% */
  taxRate: varchar("taxRate", { length: 8 }).default("0"),
  warranty: varchar("warranty", { length: 256 }),
  completionTime: varchar("completionTime", { length: 128 }),
  /** JSON array of selling point strings */
  sellingPoints: text("sellingPoints"),
  /** Subtotal in cents (auto-computed from line items, stored for quick reads) */
...
/**
 * estimate_line_items — individual parts/labor within an option
 */
export const estimateLineItems = mysqlTable("estimate_line_items", {
  ...
  /** Part quality: Aftermarket / OEM / Premium */
  partQuality: mysqlEnum("partQuality", ["Aftermarket", "OEM", "Premium"]).default("OEM"),
  laborHours: varchar("laborHours", { length: 16 }),
  /** Part cost in cents */
  partCostCents: int("partCostCents").default(0).notNull(),
  /** Labor cost in cents */
  laborCostCents: int("laborCostCents").default(0).notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});
