...
## Fix Homepage LCP Element Render Delay (1,790ms → <3s)
- [x] Removed absolute positioning from hero div (was blocking LCP paint)
- [x] Hero section now renders in normal document flow
- [x] Hero image: loading="eager", fetchPriority="high", decoding="sync", width/height defined
- [x] Preload links optimized: mobile + desktop images only (no video preload)
- [x] Critical hero CSS inlined in style block (no render-blocking)
- [x] HeroBackground component renders immediately (no state gate)
- [x] Video still plays on top of static image
- [x] Verified: design, layout, video, SEO, headings, links, tracking all unchanged
- [x] TypeScript clean (0 errors)
- [ ] Publish to production and test PageSpeed mobile
